    ...
    public static void main(String[] args) {
        ...
        for (int l = 0; l < qpath.length; l++) {
            ...
            for (int n = 0; n < graph.getRoute().size(); n++) {
                ...
                if (!startingpoint.contains(station[0])) { //If there is no starting point
                System.out.println("This path doesnt start at the starting station!");
                }
                else if(!destination.contains(station[1])) { //If there is no destination print 
                System.out.println("This path doesnt end at the destination!");
                }
                else if(!destination.contains(station[0]) && !destination.contains(station[1])){
                System.out.println("There is no train from "+ station[0] + " to " + station[1]);
                }
                else{
        ...
        if (!startingpoint.contains(user_path[0])) { //If there is no starting point
            ...
            (!destination.contains(user_path[1])) { //If there is no destination print 
            System.out.println("This path doesnt end at the destination!");
        }else if
            (!destination.contains(user_path[0]) && !destination.contains(user_path[1])){
